...
export const APPLICATION_TYPES = ['FUND', 'MENTOR', 'EVENT'] as const

export function isApplicationType(x: unknown): x is ApplicationType {
  return typeof x === 'string' && (APPLICATION_TYPES as readonly string[]).includes(x)
}
 ...
 */
export function statusToVariant(status: ApplicationStatus): 'muted' | 'amber' | 'green' | 'red' | 'purple' {
  switch (status) {
    case 'PENDING':
      return 'amber'
    case 'APPROVED':
